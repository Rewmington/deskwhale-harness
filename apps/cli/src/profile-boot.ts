...
export interface RunProfileOptions {
  ...
  /**
   * Skip the post-boot HMR watch setup (config hot-reload + the hmr/timer
   * plugin mounts). Embedded surfaces like the desktop shell cannot run the
   * HMR service — it needs Node internals (`--expose-internals`) unavailable
   * under Electron — and their config is not hot-edited anyway.
   */
  disableHmrWatch?: boolean
}
