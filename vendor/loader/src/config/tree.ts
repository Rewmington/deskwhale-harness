  ...
  import(name: string, getOuterStack?: () => string[]) {
    ...
    return composeError(async (info) => {
      ...
      if (this.ctx.loader.internal) {
        ...
        // No Node internal loader (e.g. an Electron main process, where
        // node-addon-require-builtin cannot load): resolve the bare specifier
        // from the profile baseUrl with the CJS resolver instead, which honors
        // exports maps. Only bare names reach this branch — relative paths
        // keep their URL join above, and standard Node keeps the internal path.
        const require = createRequire(fileURLToPath(new URL('./package.json', this.ctx.baseUrl!)))
