...
export namespace ModuleLoader {
  ...
  export function fromInternal(): ModuleLoader | undefined {
    ...
    // Electron's Node is patched: the CascadedLoader import pipeline can hang,
    // so embedded surfaces must not reach it. The loader's EntryTree.import
    // falls back to CJS-based resolution against baseUrl in that case.
